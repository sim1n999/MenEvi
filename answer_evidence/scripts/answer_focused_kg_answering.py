"""Run answer-focused KG answering from evidence packets."""
...
from eval_v2 import load_items, save_json, score_row_v2, write_csv, compute_metrics_v2
...
class TextGenerator:
    def __init__(self, model_path: str, load_in_4bit: bool = True, dtype: str = "bfloat16"):
        ...
        self.backend = "glm4v" if model_type.startswith("glm4v") else "causal_lm"

        if self.backend == "glm4v":
            from transformers import AutoProcessor

            try:
                if model_type == "glm4v_moe":
                    from transformers import Glm4vMoeForConditionalGeneration as model_cls
                else:
    ...
    def generate(self, prompt: str, max_new_tokens: int = 64) -> Dict[str, Any]:
        ...
        output = decoder.decode(new_ids, skip_special_tokens=True)
...
def main() -> None:
    ...
    print(f"Wrote answer-focused KG predictions to {out_dir}")
