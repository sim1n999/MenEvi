...
try:
    ...
    from transformers import AutoModelForMultimodalLM as Glm4vForConditionalGeneration
...
class GLM46VModel(LLM):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.is_flash_model = "flash" in str(model_name).lower()
        ...
        model_kind = "dense Flash" if self.is_flash_model else "MoE"
        logger.info(f"[GLM46V] Loading {model_kind} model: {model_name}, thinking={self.enable_thinking}")
        ...
        # ── Model ──
        # GLM-4.6V-Flash is a dense glm4v checkpoint; GLM-4.6V is MoE.
        # Use sdpa by default so older-glibc servers do not need flash-attn.
        model_cls = Glm4vForConditionalGeneration if self.is_flash_model else Glm4vMoeForConditionalGeneration
